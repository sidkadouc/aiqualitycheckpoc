...
@description('Whether the vault is reachable from the public internet. Set to Disabled when fronting with a private endpoint.')
@allowed(['Enabled', 'Disabled'])
param publicNetworkAccess string = 'Enabled'

// Key Vault names are globally unique; append a short RG-scoped hash.
var nameSuffix = take(uniqueString(resourceGroup().id), 6)

resource keyVault 'Microsoft.KeyVault/vaults@2023-07-01' = {
  name: 'kv-${environmentName}-${nameSuffix}'
  location: location
  tags: tags
  properties: {
    sku: {
